...
// Сохраняет объекты, которые нам нужны при обратном вызове,
// чтобы определить, были ли все данные считаны из сокета,
//  и при необходимости инициировать следующую операцию асинхронного чтения.
struct Session {
...
//Функция, используемая в качестве обратного вызова для
// операции асинхронного чтения. Проверяет, все ли данные
// были считаны из сокета, и при необходимости инициирует новую операцию чтения.
...
void readFromSocket(std::shared_ptr<asio::ip::tcp::socket> sock)
{
    ...
    const unsigned int MESSAGE_SIZE = 15;
