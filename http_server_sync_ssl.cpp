...
double
    get_intersection_coefficient(std::map<int, int> &sku_id_prev, std::map<int, int> &sku_id_next,
                                 std::size_t sku_size) {
        double coefficient = 0;
        for (auto &&[k, v] : sku_id_prev) {
            
            if (auto it = sku_id_next.find(k); it != sku_id_next.end()) {
                coefficient += std::min(v, it->second) * 2;
            }
        
        }
        coefficient /= (sku_size);
        return coefficient;
    }